class TestBench_efp_point_multiplier(TB_aluEC_prime):

    def add_point_multiplication(self, dp, P, n, R):
        self.add_action( "func(" + dp.ec1_point_to_vhdl_string(P) +", -- P" )
        self.add_action( "     " + dp.Fp_element_to_vhdl_string(n) +", -- n" )
        self.add_action( "     " + dp.ec1_point_to_vhdl_string(R) +"); -- R" )
...
class Eefp_point_multiplier(Entity):
    def __init__(self, dp):
        self.id = "ec/efp_point_multiplier"
        self.src = [ "efp_point_multiplier.vhd" ]
        self.dep = [ "ec/efp_point_adder_doubler" ]

    def is_compatible_with(self, dp):
        return True

    def get_default_tb(self, dp):
        return TestBench_efp_point_multiplier(dp, self)

obj = Eefp_point_multiplier(dp)