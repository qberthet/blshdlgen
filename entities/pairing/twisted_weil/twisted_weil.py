class TestBench_twisted_weil_pairing(TB_aluEC_prime_ext):

    def add_weil(self, P, Q, n, r):
        self.add_action( "func(" + self.dp.ec1_point_to_vhdl_string(P)  + ",  -- P" )
        self.add_action( "     " + self.dp.ec1_point_to_vhdl_string(Q)  + ",  -- Q" )
    ...
    def __init__(self, dp, entity):
        ...
        self.add_func_param( "ecQ : ec1_point" )
        ...
        for i in range(op_count):
            ...
            # Q = dp.E.gen(1) * randrange(2**50)
            Q = dp.E1.gen(0) * randrange(2**50)
            # FIXME find anther way, to slow
            n = P.order()
            # Twist P.x
            PE2 = dp.E2(P.xy())
            QE2 = dp.E2(Q.xy())
            PE2x,PE2y = PE2.xy()
            PE2twisted = dp.E2(PE2x*dp.z,PE2y)
            # FIXME catch division by zero ?
            r = PE2twisted.weil_pairing(QE2, n)
            self.add_weil(P, Q, n, r)
        self.add_action( "" )
        self.add_action( "-- Testbench actions end" )

class Twisted_weil_pairing(Entity):
    def __init__(self, dp):
        self.id = "pairing/twisted_weil"
        self.src = [ "twisted_weil.vhd" ]
        self.dep = [
    ...
    def is_compatible_with(self, dp):
        if hasattr(dp, 'E1') and hasattr(dp, 'E2'):
            return True
        else:
            return False

    def get_default_tb(self, dp):
        return TestBench_twisted_weil_pairing(dp, self)

obj = Twisted_weil_pairing(dp)
