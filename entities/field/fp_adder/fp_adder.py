class TestBench_fp_adder(TB_aluXYZ_prime):
    def __init__(self, dp, entity):
        TB_aluXYZ_prime.__init__(self, dp, entity)
        op_count = 10
        self.add_action( "-- Testbench actions" )
        self.add_action( "" )
        self.add_action( "-- " + str(op_count) + " additions of random " + str(dp.p.nbits())  + " bits elements" )
...
class Fp_adder(Entity):
    def __init__(self, dp):
        self.id = "field/fp_adder"
        self.src = [ "fp_adder.vhd" ]
    ...
    def get_default_tb(self, dp):
        return TestBench_fp_adder(dp, self)
